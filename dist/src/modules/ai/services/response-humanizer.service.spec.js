...
const response_humanizer_service_1 = require("./response-humanizer.service");
...
describe('ResponseHumanizerService', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            providers: [response_humanizer_service_1.ResponseHumanizerService],
        }).compile();
        service = module.get(response_humanizer_service_1.ResponseHumanizerService);
    });
    ...
    describe('humanizeResponse', () => {
        ...
        it('runs full pipeline and softens tone', () => {
            ...
            const ctx = {
                brandTone: tones_1.ToneProfile.WARM_PROFESSIONAL,
        ...
        it('simplifies long promotional greeting to short template', () => {
            const raw = "I've got you 💛 Thank you for contacting Fiesta House Maternity, Kenya's leading luxury photo studio. We specialize in professional maternity photography.";
            const result = service.humanizeResponse(raw, {
                pipeline: ['NATURALNESS'],
                isFirstMessage: true,
            });
            expect(result).toBe(naturalness_1.GREETING_SIMPLE_TEMPLATE);
        });
        it('skips unknown pipeline steps without throwing', () => {
            const result = service.humanizeResponse('Hello', {
                pipeline: ['SOFTEN', 'UNKNOWN', 'EMOJI'],
            });
    ...
    describe('softenTone', () => {
        it('applies formal → conversational replacements', () => {
            ...
            expect(service.softenTone('Please reply confirm to proceed.')).toContain("If everything looks good, just reply confirm and I'll take care of the rest");
    ...
    describe('injectEmoji', () => {
        it('appends 1–2 emojis from allowed set for WARM_PROFESSIONAL', () => {
            ...
            const allowed = emoji_1.EMOJI_CONTEXT_MAP.BOOKING_SUCCESS;
        ...
        it('does not add emojis for CLINICAL_FORMAL (emojiFrequency none)', () => {
            ...
            const result = service.injectEmoji(text, {
                brandTone: tones_1.ToneProfile.CLINICAL_FORMAL,
                isBookingFlow: true,
            });
            expect(result).toBe(text);
        });
    ...
    describe('applyVariation', () => {
        it('skips opener/closer when isEscalation', () => {
            ...
            expect(result).toBe(text);
            expect(phrases_1.WARM_OPENERS.every((o) => !result.startsWith(o))).toBe(true);
            expect(phrases_1.WARM_CLOSERS.every((c) => !result.endsWith(c))).toBe(true);
        });
        ...
        it('can add opener/closer for package_inquiry when no recent use', () => {
            ...
            const hasOpener = phrases_1.WARM_OPENERS.some((o) => result.startsWith(o));
            const hasCloser = phrases_1.WARM_CLOSERS.some((c) => result.includes(c));
            expect(hasOpener || hasCloser).toBe(true);
        });
        it('avoids repeating opener that appears in lastAiResponses', () => {
            const usedOpener = phrases_1.WARM_OPENERS[0];
...
//# sourceMappingURL=response-humanizer.service.spec.js.map