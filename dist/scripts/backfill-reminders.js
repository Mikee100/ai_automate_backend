...
async function main() {
    ...
    for (const booking of bookingsNeedingReminders) {
        try {
            const bookingDateTime = luxon_1.DateTime.fromJSDate(booking.dateTime).setZone(STUDIO_TZ);
            const now = luxon_1.DateTime.now().setZone(STUDIO_TZ);
            ...
            for (const reminder of remindersToSchedule) {
                ...
                await remindersQueue.add('send-reminder', { reminderId: reminderRecord.id }, { delay });
                ...
                console.log(`✅ Scheduled ${reminder.type} reminder for booking ${booking.id} (${bookingTimeStr}) - will send at ${reminderTimeStr}`);
            }
            scheduled += remindersToSchedule.length;
        }
        catch (error) {
...
//# sourceMappingURL=backfill-reminders.js.map