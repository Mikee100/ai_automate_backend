...
async function main() {
    ...
    try {
        ...
        if (!selectedService) {
            ...
            if (!packageExists) {
                const packages = await prisma.package.findMany();
                throw new Error(`Package "${selectedService}" not found. Available packages: ${packages.map(p => p.name).join(', ')}`);
            }
        ...
        if (bookingDate && bookingTime) {
            ...
            const STUDIO_TZ = 'Africa/Nairobi';
            const tomorrow = luxon_1.DateTime.now().setZone(STUDIO_TZ).plus({ days: 1 }).set({ hour: 14, minute: 0, second: 0, millisecond: 0 });
        ...
        console.log(`   Date/Time: ${luxon_1.DateTime.fromJSDate(bookingDateTime).setZone('Africa/Nairobi').toFormat('yyyy-MM-dd HH:mm')} (Africa/Nairobi)`);
...
//# sourceMappingURL=create-fresh-booking.js.map